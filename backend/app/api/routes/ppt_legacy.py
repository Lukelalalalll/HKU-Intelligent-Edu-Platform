...
async def stream_events(project_id: str, request_id: str | None = Query(default=None), last_event_id: str | None = Header(default=None, alias="Last-Event-ID"), service: PptAgentService = Depends(svc)):
    service.project(project_id)
    current = int(last_event_id or 0) if (last_event_id or "0").isdigit() else 0
    user_id = service.user.id
    # The dependency session is only needed for the initial authorization
    # check; do not retain it for the lifetime of the streaming response.
    service.db.close()
    async def gen():
        nonlocal current
        for tick in range(1200):
            # Never keep the request-scoped SQLAlchemy session alive while the
            # client is connected. Each poll uses a short-lived read session.
            from app.db.session import SessionLocal
            with SessionLocal() as db:
                rows = list(db.scalars(select(PptAgentEvent).where(PptAgentEvent.project_id == project_id, PptAgentEvent.id > current).order_by(PptAgentEvent.id.asc()).limit(100)))
                project = db.scalar(select(PptProject).where(PptProject.id == project_id, PptProject.owner_id == user_id))
            if not project:
                return
            for row in rows:
                current = row.id
