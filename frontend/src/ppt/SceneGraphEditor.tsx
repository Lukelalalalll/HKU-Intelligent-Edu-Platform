...
import { SLIDE_HEIGHT, SLIDE_WIDTH, SlideRenderer } from "./SlideRenderer";
...
function addElement(canvas: fabric.Canvas, element: PptSlideElement): Promise<PptFabricObject | null> {
  ...
  if (element.type === "image" && element.src) {
    return new Promise((resolve) => {
      ...
      // rendering. Fabric's image loader does not consistently call back on
      // every browser/network failure, so keep a short local timeout.
      const timeout = window.setTimeout(() => finish(null), 1800);
      try {
        fabric.Image.fromURL(element.src as string, (image: fabric.Image) => {
          window.clearTimeout(timeout);
          if (!image) return finish(null);
...
export default function SceneGraphEditor({ document, editable = true, onChange, onUploadImage }: Props) {
  const canvasElement = useRef<HTMLCanvasElement | null>(null); const host = useRef<HTMLDivElement | null>(null); const stage = useRef<HTMLDivElement | null>(null); const canvasRef = useRef<fabric.Canvas | null>(null); const hydrating = useRef(false); const renderId = useRef(0); const initialized = useRef(false); const history = useRef<PptDocument[]>([]); const future = useRef<PptDocument[]>([]); const beforeChange = useRef<PptDocument | null>(null); const [selected, setSelected] = useState<PptFabricObject[]>([]); const [editing, setEditing] = useState(false); const [zoom, setZoom] = useState(1.1); const zoomRef = useRef(1.1); const [showGrid, setShowGrid] = useState(false); const [propertiesCollapsed, setPropertiesCollapsed] = useState(false); const [fallbackVisible, setFallbackVisible] = useState(false); const [liveDocument, setLiveDocument] = useState(document); const fileRef = useRef<HTMLInputElement | null>(null);
  ...
  const fitCanvas = () => {
    ...
    // The slider is a multiplier on the available fit size. A default 1.1
    // gives the slide a little more presence while the scroll container keeps
    // the full slide reachable on shorter or narrower screens.
  ...
  const insertImage = async (file: File) => { if (!onUploadImage) return; const asset = await onUploadImage(file); const canvas = canvasRef.current; if (!canvas) return; snapshot(); const current = imageActive && active ? elementFromObject(active) : null; const element: PptSlideElement = { id: current?.id || `image-${Date.now()}`, type: "image", x: current?.x ?? 56, y: current?.y ?? 24, w: current?.w ?? 34, h: current?.h ?? 42, rotation: current?.rotation, src: asset.url, asset_id: asset.id, natural_width: asset.width, natural_height: asset.height, object_fit: current?.object_fit || "contain", crop: current?.crop, alt: asset.alt || asset.name, lock_aspect_ratio: current?.lock_aspect_ratio !== false }; if (current && active) canvas.remove(active); await addElement(canvas, element); canvas.setActiveObject(canvas.getObjects()[canvas.getObjects().length - 1]); canvas.renderAll(); emit(); };
  ...
  return <div className="ppt-scene-editor">
    <div className="ppt-scene-toolbar" role="toolbar" aria-label="PowerPoint 编辑工具栏">
      ...
      <div className="ppt-scene-tool-group ppt-scene-toolbar-end"><button className={showGrid ? "active" : ""} onClick={() => setShowGrid((value) => !value)}>网格</button><label>缩放 <input type="range" min="0.75" max="1.35" step="0.05" value={zoom} onChange={(event) => setZoom(Number(event.target.value))} /></label></div>
