...
import { courseMaterialsApi, getApiErrorMessage, assignmentsApi, type Assignment, type CourseChapter, type CourseMaterial, type FileAsset, type MaterialKind, type Participant } from "../../../api";
...
export function ParticipantModule({ participants, loading, error, isTeacher, currentUserId }: { participants: Participant[]; loading: boolean; error: boolean; isTeacher: boolean; currentUserId?: string }) {
...
export function MaterialsModule({ courseId, kind, chapters, loading, error, selectedChapterId, onSelectChapter, onRefresh, isTeacher }: { courseId: string; kind: MaterialKind; chapters: CourseChapter[]; loading: boolean; error: boolean; selectedChapterId: string | null; onSelectChapter: (id: string) => void; onRefresh: () => Promise<void>; isTeacher: boolean }) {
  ...
  const createChapter = async () => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "创建 chapter 失败")); }
  ...
  const renameChapter = async (chapter: CourseChapter) => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "更新 chapter 失败")); }
    finally { setBusy(false); }
  };
  ...
  const moveChapter = async (chapter: CourseChapter, offset: number) => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "调整 chapter 顺序失败")); }
    finally { setBusy(false); }
  };
  ...
  const deleteChapter = async (chapter: CourseChapter) => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "删除 chapter 失败")); }
    finally { setBusy(false); }
  };
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "资料上传失败")); }
    finally { setBusy(false); }
  };
  ...
  const attachAssets = async (assets: FileAsset[]) => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, "资料上传失败"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const deleteMaterial = async (material: CourseMaterial) => {
    ...
    catch (error: unknown) { toast.error(getApiErrorMessage(error, "删除资料失败")); }
    finally { setBusy(false); }
  };
...
export function AssignmentsModule({ courseId, assignments, loading, error, isTeacher, onRefresh, onGrade }: { courseId: string; assignments: Assignment[]; loading: boolean; error: boolean; isTeacher: boolean; onRefresh: () => Promise<void>; onGrade: (id: string) => void }) {
...
function MaterialCard({ courseId, material, isTeacher, busy, onDelete }: { courseId: string; material: CourseMaterial; isTeacher: boolean; busy: boolean; onDelete: () => void }) {
  const download = async () => {
    ...
    } catch (error: unknown) { toast.error(getApiErrorMessage(error, "文件下载失败")); }
  };
  const processing = material.processing_status && !["ready", "partial_ready"].includes(material.processing_status);
  return <article className={styles.materialCard}><div className={styles.materialIcon}><i className={`fas ${material.mime_type.includes("pdf") ? "fa-file-pdf" : material.mime_type.startsWith("image/") ? "fa-file-image" : "fa-file-lines"}`} /></div><div className={styles.materialInfo}><strong title={material.title}>{material.title}</strong><span>{material.file_name}</span><small>{formatBytes(material.size_bytes)} · {new Date(material.uploaded_at).toLocaleDateString("zh-CN")}</small>{processing ? <small>⏳ 正在处理：{material.processing_status}</small> : material.processing_status === "partial_ready" ? <small>✓ 已就绪（降级解析）</small> : material.processing_status === "ready" ? <small>✓ 已建立知识库</small> : null}</div><div className={styles.materialActions}><button type="button" className="secondary-action" onClick={() => void download()}><i className="fas fa-download" /> 下载</button>{isTeacher && <button type="button" className={styles.iconAction} title="删除" disabled={busy} onClick={onDelete}><i className="fas fa-trash" /></button>}</div></article>;
}
